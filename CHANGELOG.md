...
- `setup-lab.sh` — Phase 2 provider-bug fallback untaints `module.vks.kubernetes_manifest.kubernetes_cluster` before retrying, so the retry no longer destroys and recreates the cluster
